...
void SymTable::deleteScope() {
    ...
    if (s_stack.top()->s_type == SCOPE_GLOBAL) {
        ...
        for (const SymPtr &basic_sym : s_stack.top()->syms) {
            assert(FUNCTION_TYPE != basic_sym->g_type);
            std::string type = TypeToString(basic_sym->g_type);
...
void SymTable::addFuncScope(TypesOfScopes scope_type, Type ret_type, FuncSymTypePtr function_symbol) {
    assert(!s_stack.empty());
